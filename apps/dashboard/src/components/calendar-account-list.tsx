...
interface CalendarAccountListProps {
  accounts: Account[];
  removeAction: (accountName: string) => Promise<{ success: boolean }>;
}

export function CalendarAccountList({ accounts, removeAction }: CalendarAccountListProps) {
  ...
  function handleAdd() {
    ...
    if (!name) return;
    window.location.href = `/api/oauth/calendar?account=${encodeURIComponent(name)}`;
  }
      ...
      {accounts.length === 0 ? (
        <div className="border border-dashed border-border rounded-sm py-12 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">
            Add an account to get started with Google Calendar integration.
          </p>
        </div>
      ) : (
