...
interface GSlidesAccountListProps {
  accounts: Account[];
  removeAction: (accountName: string) => Promise<{ success: boolean }>;
}

export function GSlidesAccountList({ accounts, removeAction }: GSlidesAccountListProps) {
  ...
  function handleAdd() {
    ...
    if (!name) return;
    window.location.href = `/api/oauth/gslides?account=${encodeURIComponent(name)}`;
  }
      ...
      {accounts.length === 0 ? (
        <div className="border border-dashed border-border rounded-sm py-12 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">
            Add an account to get started with Google Slides integration.
          </p>
        </div>
      ) : (
