...
import { GDriveAccountList } from "@/components/gdrive-account-list";
...
async function getAccounts() {
  ...
  const gdriveNames = allNames.filter((n) => n.startsWith("gdrive:"));

  return Promise.all(
    gdriveNames.map(async (tokenKey) => {
      const name = tokenKey.replace("gdrive:", "");
      try {
...
async function removeAccount(accountName: string): Promise<{ success: boolean }> {
  ...
  tokenStore.delete(`gdrive:${accountName}`);
  return { success: true };
}
    ...
    <div>
      ...
      <div className="mb-8 animate-fade-in-up">
        ...
        <p className="text-sm text-muted-foreground">
          Manage authenticated Google Drive accounts. Shared with the CLI.
        </p>
      </div>
      ...
      <div className="animate-fade-in-up" style={{ animationDelay: "100ms" }}>
        <GDriveAccountList accounts={accounts} removeAction={removeAccount} />
      </div>
    </div>
  );
}
