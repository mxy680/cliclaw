...
import { GSheetsAccountList } from "@/components/gsheets-account-list";
...
async function getAccounts() {
  ...
  const gsheetsNames = allNames.filter((n) => n.startsWith("gsheets:"));

  return Promise.all(
    gsheetsNames.map(async (tokenKey) => {
      const name = tokenKey.replace("gsheets:", "");
      try {
...
async function removeAccount(accountName: string): Promise<{ success: boolean }> {
  ...
  tokenStore.delete(`gsheets:${accountName}`);
  return { success: true };
}

export default async function GSheetsPage({
  searchParams,
}: {
    ...
    <div>
      ...
      <div className="mb-8 animate-fade-in-up">
        ...
        <p className="text-sm text-muted-foreground">
          Manage authenticated Google Sheets accounts. Shared with the CLI.
        </p>
      </div>
      ...
      <div className="animate-fade-in-up" style={{ animationDelay: "100ms" }}>
        <GSheetsAccountList accounts={accounts} removeAction={removeAccount} />
      </div>
    </div>
  );
}
